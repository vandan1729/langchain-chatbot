...
st.write(
    "[![view source code ](https://img.shields.io/badge/view_source_code-gray?logo=github)](https://github.com/vandan1729/langchain-chatbot/blob/master/pages/5_%F0%9F%9B%A2_chat_with_sql_db.py)"
)
    ...
    def main(self):
        ...
        if radio_opt.index(selected_opt) == 1:
            with st.sidebar.popover(":orange[⚠️ Security note]", use_container_width=True):
                ...
                st.warning(warning)
            db_uri = st.sidebar.text_input(label="Database URI", placeholder="mysql://user:pass@hostname:port/db")
        else:
